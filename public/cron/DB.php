...
class DB{
    ...
    private $db_name = "hr_system";	// Change as required
    ...
    public function getRowCount($query){
        $r = $this->myconn->query($query) or die($this->myconn->error . __LINE__);
        return $r->num_rows;
    }

    public function getOneRecord($query)
    {
        $r = $this->myconn->query($query.' LIMIT 1') or die($this->conn->error.__LINE__);
